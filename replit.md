...
- Sync flow: fetch v4 entity maps (incl. adGroups+adGroupAds for video IDs, advertiser timezone) → fetch YouTube oEmbed metadata → run slimmed Bid Manager report → parse CSV + backfill dimensions from v4 metadata → download creative assets → upsert into `dv360_raw_performance`.
