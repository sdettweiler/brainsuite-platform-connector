...
The DV360 integration uses two separate Google APIs with a **two-report YOUTUBE architecture + v4 backfill**:
...
- **Bid Manager API v2** (`doubleclickbidmanager.googleapis.com/v2`): **Two-report YOUTUBE type** architecture. YouTube campaigns require `type: "YOUTUBE"` (not `type: "STANDARD"`) which uses `VIDEO_*` metric names instead of `RICH_MEDIA_*`. Conversion metrics cannot be mixed with non-conversion metrics in a single YouTube query, so two reports are used:
  - **Report 1 (perf)**: 19 non-conversion metrics — `METRIC_IMPRESSIONS`, `METRIC_CLICKS`, `METRIC_CLICK_RATE`, `METRIC_MEDIA_COST_ADVERTISER`, `METRIC_VIDEO_VIEWS`, `METRIC_VIDEO_VIEWS_RATE`, `METRIC_VIDEO_COMPLETE_IMPRESSIONS`, `METRIC_VIDEO_FIRST/MIDPOINT/THIRD_QUARTILE_IMPRESSIONS`, `METRIC_VIDEO_SKIPS`, `METRIC_COST_PER_VIDEO_VIEW`, `METRIC_ACTIVE_VIEW_MEASURABLE/VIEWABLE/PERCENT_VIEWABLE_IMPRESSIONS`, `METRIC_VIDEO_COMPANION_IMPRESSIONS/CLICKS`, `METRIC_BILLABLE_IMPRESSIONS`, `METRIC_ENGAGEMENTS`.
  - **Report 2 (conv)**: 4 conversion-only metrics — `METRIC_TOTAL_CONVERSIONS`, `METRIC_POST_VIEW_CONVERSIONS`, `METRIC_POST_CLICK_CONVERSIONS`, `METRIC_REVENUE_CONVERSION_COST_ADVERTISER`. Fails gracefully if Floodlight isn't configured.
  - Both reports use 8 groupBys: `FILTER_DATE`, `FILTER_ADVERTISER`, `FILTER_ADVERTISER_NAME`, `FILTER_ADVERTISER_CURRENCY`, `FILTER_INSERTION_ORDER`, `FILTER_LINE_ITEM`, `FILTER_LINE_ITEM_TYPE`, `FILTER_YOUTUBE_AD_VIDEO_ID`.
  - Report 1 full-upserts rows. Report 2 merges conversion columns into existing rows (UPDATE only, never overwrites performance data).
...
- **YouTube Video ID retrieval**: Primary source is now the Bid Manager CSV column (`FILTER_YOUTUBE_AD_VIDEO_ID`). Fallback via adGroupAds chain: `_fetch_ad_groups()` maps adGroupId→lineItemId, `_fetch_ad_group_ads()` extracts YouTube video IDs. Handles `videoPerformanceAd.videos[]` array format (multiple videos per ad) and `commonInStreamAttribute.video.id` single format. `video_metadata` flat dict maps each video ID to its ad_type_label for creative_type backfill. `line_item_videos` stores all video IDs per line item (not just first).
...
- **Cookie validation**: `_check_youtube_cookies()` validates YOUTUBE_COOKIES env var before yt-dlp attempts: checks Netscape cookie format expiry timestamps, returns valid/expired/missing. Expired/missing cookies skip video downloads but still download thumbnails.
- **Asset-level aggregation**: `ad_id` = YouTube video ID (matching Meta/TikTok pattern where ad_id = platform creative identifier). Multiple line items sharing the same video aggregate metrics (sum additive, recalculate derived) into one row per video per date. This creates a 1:1 mapping between YouTube video ID and CreativeAsset via `uq_creative_asset` constraint.
- **New fields**: `video_skips` (Integer, on `Dv360RawPerformance` only — stored in `platform_extras` JSON during harmonization). `cost_per_view` (Decimal, now populated from `METRIC_COST_PER_VIDEO_VIEW` — maps to harmonized `cpv` and `cost_per_view`). `video_plays` is intentionally `None` (no YouTube "starts" metric — `METRIC_VIDEO_VIEWS` counts views, not starts).
- Sync flow: fetch v4 entity maps → run Report 1 (perf) + Report 2 (conv) → fetch YouTube oEmbed metadata → parse CSVs + backfill from v4 → upsert perf rows → merge conv data → download creative assets.
...
## Recent Changes
- 2026-02-27: Two-report YOUTUBE architecture fully implemented — Both Report 1 (19 perf metrics) and Report 2 (4 conversion metrics) now run live via Bid Manager API v2 with `type: "YOUTUBE"`. Conversion report no longer returns empty; it submits a real query and polls for results. Added `cost_per_view` column to Dv360RawPerformance (migration k2l3m4n5o6p7). Added `video_skips` column (migration j1k2l3m4n5o6). CSV parsing updated for YouTube column names: "Video Views", "View Rate", "Video Completions"/"Completion", "First Quartile", "Midpoint", "Third Quartile", "Skips", "CPV", "Active View: Measurable/Viewable/% Viewable Impressions", "Companion Impressions/Clicks", "Billable Impressions", "Engagements". Derived metrics: video_completion_rate (completions/views or completions/(completions+skips)), engagement_rate, cpm, cpc. All additive fields properly aggregated across multi-line-item video IDs. Adaptive polling (30s→60s→120s intervals, 2-hour max wait) with automatic OAuth token refresh handles long-running YouTube reports. Report 2 (conversion) fails gracefully if Floodlight isn't configured.
