...
The DV360 integration uses two separate Google APIs with a **slimmed report + v4 backfill** architecture:
...
- **Bid Manager API v2** (`doubleclickbidmanager.googleapis.com/v2`): **Slimmed report** with 8 groupBys: `FILTER_DATE`, `FILTER_ADVERTISER`, `FILTER_ADVERTISER_NAME`, `FILTER_ADVERTISER_CURRENCY`, `FILTER_INSERTION_ORDER`, `FILTER_LINE_ITEM`, `FILTER_LINE_ITEM_TYPE`, `FILTER_YOUTUBE_AD_VIDEO_ID`. The 10 removed groupBys (timezone, ad position/type, IO goal type, creative ID/type/source, media plan/name, media type) were incompatible with YouTube Video ID and are backfilled from DV360 v4 API entity metadata. With those removed, FILTER_YOUTUBE_AD_VIDEO_ID now works and provides per-video metrics directly. 25 metrics unchanged.
...
- Sync flow: fetch v4 entity maps (incl. adGroups+adGroupAds for video IDs, advertiser timezone) → fetch YouTube oEmbed metadata → run slimmed Bid Manager report → parse CSV + backfill dimensions from v4 metadata → aggregate by video ID per date → download creative assets → upsert into `dv360_raw_performance`.
...
## Recent Changes
- 2026-02-27: FILTER_YOUTUBE_AD_VIDEO_ID restored + asset-level aggregation — Added FILTER_YOUTUBE_AD_VIDEO_ID back to Bid Manager groupBys (now 8 total) since the 10 incompatible filters were removed. Report now gives per-video metrics directly from CSV. Changed `ad_id` from `{line_item_id}_{date}` to YouTube video ID (from CSV column, fallback to adGroupAds). `line_item_videos` now stores ALL video IDs per line item (not just first). New `video_metadata` flat dict maps each video ID to its ad_type_label for creative_type backfill. Fresh YouTube cookies stored cleanly.
