...
- **Bid Manager API v2** (`doubleclickbidmanager.googleapis.com/v2`): Two YOUTUBE-type reports:
  - **Report 1 (Performance)**: 19 YouTube-compatible metrics:
    - **Core**: `METRIC_IMPRESSIONS`, `METRIC_CLICKS`, `METRIC_CLICK_RATE`
    - **Spend**: `METRIC_MEDIA_COST_ADVERTISER`
    - **Video**: `METRIC_VIDEO_VIEWS`, `METRIC_VIDEO_VIEWS_RATE`, `METRIC_VIDEO_COMPLETE_IMPRESSIONS`, `METRIC_VIDEO_FIRST_QUARTILE_IMPRESSIONS`, `METRIC_VIDEO_MIDPOINT_IMPRESSIONS`, `METRIC_VIDEO_THIRD_QUARTILE_IMPRESSIONS`, `METRIC_VIDEO_SKIPS`
    - **Cost**: `METRIC_COST_PER_VIDEO_VIEW`
    - **Active View**: `METRIC_ACTIVE_VIEW_MEASURABLE_IMPRESSIONS`, `METRIC_ACTIVE_VIEW_VIEWABLE_IMPRESSIONS`, `METRIC_ACTIVE_VIEW_PERCENT_VIEWABLE_IMPRESSIONS`
    - **Companions**: `METRIC_VIDEO_COMPANION_IMPRESSIONS`, `METRIC_VIDEO_COMPANION_CLICKS`
    - **Billable**: `METRIC_BILLABLE_IMPRESSIONS`
    - **Engagement**: `METRIC_ENGAGEMENTS`
  - **Report 2 (Conversion)**: 4 conversion-only metrics (cannot mix with non-conversion):
    - `METRIC_TOTAL_CONVERSIONS`, `METRIC_POST_VIEW_CONVERSIONS`, `METRIC_POST_CLICK_CONVERSIONS`, `METRIC_REVENUE_CONVERSION_COST_ADVERTISER`
    - Fails gracefully if Floodlight not configured (returns [])
  - Both use 8 groupBys: `FILTER_DATE`, `FILTER_ADVERTISER`, `FILTER_ADVERTISER_NAME`, `FILTER_ADVERTISER_CURRENCY`, `FILTER_INSERTION_ORDER`, `FILTER_LINE_ITEM`, `FILTER_LINE_ITEM_TYPE`, `FILTER_YOUTUBE_AD_VIDEO_ID`.
  - **Derived metrics** (calculated locally): `cpm`, `cpc`, `ctr`, `video_view_rate`, `video_completion_rate`, `engagement_rate`, `active_view_viewability`.
...
- **Asset-level aggregation**: `ad_id` = YouTube video ID (matching Meta/TikTok pattern where ad_id = platform creative identifier). Multiple line items sharing the same video aggregate metrics (sum additive, recalculate derived) into one row per video per date. Additive fields: spend, impressions, clicks, trueview_views, video_views, video_completions, video quartiles, video_skips, companion impressions/clicks, active_view metrics, billable_impressions, engagements.
- **Fields**: `cost_per_view` (from `METRIC_COST_PER_VIDEO_VIEW`), `video_skips` (from `METRIC_VIDEO_SKIPS`), `video_plays` intentionally `None` (no YouTube "starts" metric). All stored in `platform_extras` JSON for harmonization: video_skips, active_view_measurable/viewable_impressions, active_view_viewability, billable_impressions, etc.
...
## Recent Changes
- 2026-03-02: DV360 two-report YOUTUBE architecture — Expanded from 11 to 19 performance metrics using YouTube-compatible names (METRIC_VIDEO_VIEWS instead of METRIC_TRUEVIEW_VIEWS, etc.). Added video quartiles, skips, Active View, companion metrics, billable impressions, engagements. Re-enabled conversion report (Report 2) with 4 conversion metrics. Derived metrics (cpm, cpc, video_completion_rate, engagement_rate, active_view_viewability) calculated locally. Fixed asset_format: DV360 YouTube assets now set to "VIDEO" (was "DISPLAY"), fixing video playback in dashboard detail view. Added /static proxy route for development. Updated safe_float to strip % from percentage-formatted values.
