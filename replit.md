...
- **Bid Manager API v2** (`doubleclickbidmanager.googleapis.com/v2`): **Slimmed report** with 7 groupBys only: `FILTER_DATE`, `FILTER_ADVERTISER`, `FILTER_ADVERTISER_NAME`, `FILTER_ADVERTISER_CURRENCY`, `FILTER_INSERTION_ORDER`, `FILTER_LINE_ITEM`, `FILTER_LINE_ITEM_TYPE`. The 10 removed groupBys (timezone, ad position/type, IO goal type, creative ID/type/source, media plan/name, media type) are incompatible with YouTube Video ID and are now backfilled from DV360 v4 API entity metadata. 25 metrics unchanged.
...
- **YouTube Video ID retrieval**: Via adGroupAds chain: `_fetch_ad_groups()` maps adGroupId→lineItemId, `_fetch_ad_group_ads()` extracts YouTube video IDs. Handles `videoPerformanceAd.videos[]` array format (multiple videos per ad) and `commonInStreamAttribute.video.id` single format. Maps ad type labels (In-Stream, Bumper, Video Performance, etc.) for creative_type backfill.
...
## Recent Changes
- 2026-02-27: Asset-level aggregation — Changed DV360 `ad_id` from `{line_item_id}_{date}` to YouTube video ID, matching Meta/TikTok pattern where ad_id = platform creative identifier. Multiple line items sharing the same video now aggregate (sum additive metrics, recalculate derived) into one row per video per date. Creates 1:1 mapping between YouTube video ID and CreativeAsset via `uq_creative_asset`. Fresh YouTube cookies stored.
