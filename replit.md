...
- **Bid Manager API v2** (`doubleclickbidmanager.googleapis.com/v2`): **Slimmed report** with 8 groupBys and 11 metrics. GroupBys: `FILTER_DATE`, `FILTER_ADVERTISER`, `FILTER_ADVERTISER_NAME`, `FILTER_ADVERTISER_CURRENCY`, `FILTER_INSERTION_ORDER`, `FILTER_LINE_ITEM`, `FILTER_LINE_ITEM_TYPE`, `FILTER_YOUTUBE_AD_VIDEO_ID`. Metrics: impressions, clicks, CTR, eCPM, eCPC, revenue, TrueView views, video completion rate, engagements, engagement rate, TrueView view rate. 14 metrics removed for YouTube Video ID compatibility (total_media_cost, total_conversions, post-click/view conversions, all rich_media video metrics, all active_view metrics, billable_impressions, eCPA). 10 groupBys also removed (timezone, ad position/type, IO goal type, creative ID/type/source, media plan/name, media type) — all backfilled from DV360 v4 API entity metadata.
