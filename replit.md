...
## Recent Changes
- 2026-03-03: DV360 asset download fix — (1) `download_assets_post_commit()` restructured to two-pass: thumbnails first (with immediate DB commit), then videos. Ensures thumbnails always persist even when video downloads fail/timeout. (2) yt-dlp: format changed to `"b"` (best single stream), `extractor_args` with `ios`+`web` player clients to bypass YouTube n challenge without deno, socket_timeout reduced to 15s, removed merge_output_format. (3) Format-not-available errors now break immediately instead of retrying with backup cookies (same root cause, not a cookie issue).
