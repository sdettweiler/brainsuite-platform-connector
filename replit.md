...
- **Pre-uvicorn health server**: `replit_start.sh` launches a minimal Python HTTP server on port 5000 before importing the app. `backend/start_server.py` pre-imports all app modules (the slow ~10s step in production), then kills the health placeholder and starts uvicorn on the same port (instant bind since modules are cached). This eliminates health check 500s during the Python import phase.
- **Scheduler startup delay**: In production (`REPLIT_DEPLOYMENT` set), `_background_startup()` waits 15s after migrations before starting the scheduler, giving the container's outbound network/TLS stack time to initialize fully. Prevents ConnectErrors on early API calls.
...
## Disconnect & Purge
- **Two disconnect modes**: When disconnecting an ad account, users choose between "Disconnect — keep existing data" (soft delete, sets `is_active = False`) or "Disconnect and delete all data" (hard delete, removes all associated data permanently).
- **Purge service**: `backend/app/services/connection_purge.py` — `purge_connection_data(db, connection_id, org_id)` deletes all data in correct FK order: asset_metadata_values → asset_project_mappings → harmonized_performance → raw performance tables → creative_assets → sync_jobs → platform_connections. Also deletes object storage blobs referenced by URLs in the DB. Verifies org ownership before deletion. Rolls back on failure.
- **Backend**: `DELETE /connections/{id}?purge=true` for full deletion, `POST /bulk-action` with `action: "disconnect_purge"`. Default (no `purge` param) retains existing soft-delete behavior.
- **Frontend dialog**: `DisconnectDialogComponent` (`frontend/src/app/features/configuration/components/disconnect-dialog.component.ts`) — Material dialog with radio options for retain vs purge, replaces native `confirm()`. Used for both individual and bulk disconnect.
- **Object storage**: `delete_blob()`, `list_blobs()`, `delete_blobs_by_prefix()` methods added to `ObjectStorageService`.
