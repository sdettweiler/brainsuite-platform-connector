...
The DV360 integration uses two separate Google APIs with a **single YOUTUBE report + v4 backfill**:
...
- **Bid Manager API v2** (`doubleclickbidmanager.googleapis.com/v2`): Single YOUTUBE-type report with **11 API-verified metrics** (all others are incompatible with `YOUTUBE` type + `FILTER_YOUTUBE_AD_VIDEO_ID` groupBy):
  - **Core**: `METRIC_IMPRESSIONS`, `METRIC_CLICKS`, `METRIC_CTR`
  - **Spend**: `METRIC_MEDIA_COST_ADVERTISER`, `METRIC_MEDIA_COST_ECPM_ADVERTISER`, `METRIC_MEDIA_COST_ECPC_ADVERTISER`, `METRIC_REVENUE_ADVERTISER`
  ...
  - **Incompatible metrics** (all rejected by API): `VIDEO_*` quartile/completion/skips, `ACTIVE_VIEW_*`, `COMPANION_*`, `ENGAGEMENTS`, `BILLABLE_*`, and all conversion metrics (`TOTAL_CONVERSIONS`, etc.).
  - CSV column names from Google: `TrueView: Views`, `TrueView: View Rate`, `Completion Rate (Video)`, `YouTube: Revenue eCPV (Adv Currency)`, `Media Cost eCPM (Adv Currency)`, `Media Cost eCPC (Adv Currency)`, `Revenue (Adv Currency)`, `Video ID`.
...
- **YouTube Video ID retrieval**: Primary source is the Bid Manager CSV column `Video ID` (from `FILTER_YOUTUBE_AD_VIDEO_ID`). Fallback via adGroupAds chain: `_fetch_ad_groups()` maps adGroupId→lineItemId, `_fetch_ad_group_ads()` extracts YouTube video IDs. Handles `videoPerformanceAd.videos[]` array format (multiple videos per ad) and `commonInStreamAttribute.video.id` single format. `video_metadata` flat dict maps each video ID to its ad_type_label for creative_type backfill. `line_item_videos` stores all video IDs per line item (not just first).
...
- **Fields**: `cost_per_view` (Decimal, populated from `METRIC_TRUEVIEW_CPV_ADVERTISER` → maps to harmonized `cpv` and `cost_per_view`). `video_skips` (Integer, on `Dv360RawPerformance` — unused since metric incompatible). `video_plays` is intentionally `None` (no YouTube "starts" metric).
- Sync flow: fetch v4 entity maps → run YOUTUBE report (11 metrics) → fetch YouTube oEmbed metadata → parse CSV + backfill from v4 → upsert rows → download creative assets.
...
## Recent Changes
- 2026-03-02: DV360 YOUTUBE report working end-to-end — Fixed CSV column name mappings to match actual Google output: `TrueView: Views` (not `TrueView Views`), `TrueView: View Rate` (not `TrueView VR`), `Completion Rate (Video)` (not `Complete Rate (Video)`), `YouTube: Revenue eCPV (Adv Currency)` (new CPV column), `Media Cost eCPM (Adv Currency)`, `Media Cost eCPC (Adv Currency)`, `Video ID` (not `YouTube Ad Video ID`). Confirmed only 11 metrics work with YOUTUBE type + FILTER_YOUTUBE_AD_VIDEO_ID. Conversion report disabled (returns empty) since all conversion metrics are incompatible. 716 rows successfully upserted and harmonized with video_views, view_rate, CPV, eCPM, eCPC all populated.
