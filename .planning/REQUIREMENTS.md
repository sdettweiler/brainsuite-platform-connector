...
- [x] **SEC-01**: OAuth session state stored in Redis with TTL (replace in-memory `_oauth_sessions` dict)
...
- [x] **QUAL-01**: All broad `except Exception` blocks replaced with specific exception types and structured logging
...
- [x] **SCORE-01**: `creative_score_results` table with scoring state machine (UNSCORED → PENDING → PROCESSING → COMPLETE | FAILED)
...
- [x] **SCORE-04**: APScheduler scoring job runs every 15 minutes, batches up to 20 UNSCORED assets, respects rate limits
- [x] **SCORE-05**: New assets automatically queued as UNSCORED after platform sync completes
- [x] **SCORE-06**: Manual re-score trigger available per creative via UI and API endpoint
...
- [x] **SCORE-08**: Scoring status endpoint (`/scoring/status`) for frontend polling
...
- [x] **DASH-03**: Score dimension breakdown panel accessible per creative (expandable or side panel)
...
- [x] **REL-02**: Failed syncs and expired platform tokens surfaced to user with reconnect prompt
- [x] **REL-03**: APScheduler runs on exactly one worker (SCHEDULER_ENABLED env var guard for multi-worker Autoscale / cloud deployments)
