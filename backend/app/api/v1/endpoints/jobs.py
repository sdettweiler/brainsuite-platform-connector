...
async def _dispatch_job_retry(
    ...
    if job_type in ("sync_daily", "sync_full", "sync_initial", "sync_historical") and (params or {}).get("platform") not in (None, "DV360"):
        ...
        if not conn_id:
            raise ValueError(f"sync retry: missing platform_connection_id in params for job {new_job_id}")
        ...
        if not fn:
            raise ValueError(f"sync retry: unknown job_type {job_type} for job {new_job_id}")
    ...
    if job_type == "autofill":
        ...
        if not (asset_id_str and org_id_str):
            raise ValueError(f"autofill retry: missing asset_id or org_id in params for job {new_job_id}")
    ...
    if job_type == "scoring":
        ...
        if not score_id_str:
            raise ValueError(f"scoring retry: missing score_id in params for job {new_job_id}")
