...
- No auto_fill_enabled fields: COMPLETE without OpenAI call
- All 7 auto_fill_type routing paths
- OPENAI_API_KEY=None graceful no-op (D-09)
...
async def test_complete_guard():
    """Assets with COMPLETE tracking row must NOT trigger OpenAI calls."""
    ...
    with patch("app.services.ai_autofill.get_session_factory") as mock_sf, \
         patch("app.services.ai_autofill.AsyncOpenAI") as mock_openai:
...
# ---------------------------------------------------------------------------
# Test 2: No auto_fill_enabled fields → COMPLETE without OpenAI
# ---------------------------------------------------------------------------
...
async def test_language_field_routing():
    """language auto_fill_type populates from GPT-4o Vision response."""
...
async def test_vo_transcript_field_routing():
    """vo_transcript auto_fill_type populates from Whisper response text."""
    from app.services.ai_autofill import _autofill
    ...
    with patch("app.services.ai_autofill.get_session_factory") as mock_sf, \
         patch("app.services.ai_autofill.settings") as mock_settings, \
         patch("app.services.ai_autofill.get_object_storage") as mock_storage, \
         patch("app.services.ai_autofill._run_whisper", new_callable=AsyncMock) as mock_whisper, \
        ...
        mock_whisper.return_value = {"text": "Hello world", "language": "en"}
...
async def test_vo_transcript_truncation():
    ...
    with patch("app.services.ai_autofill.get_session_factory") as mock_sf, \
        ...
        mock_whisper.return_value = {"text": long_text, "language": "en"}
...
async def test_vo_language_field_routing():
    """vo_language auto_fill_type populates language from Whisper response."""
    from app.services.ai_autofill import _autofill
    ...
    with patch("app.services.ai_autofill.get_session_factory") as mock_sf, \
        ...
        mock_storage.return_value.download_blob.return_value = (b"fakevideo", "video/mp4")
        mock_audio.return_value = b"fakeaudio" * 200
        mock_whisper.return_value = {"text": "Hola", "language": "es"}
...
# ---------------------------------------------------------------------------
# Test 11: OPENAI_API_KEY=None graceful no-op
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_no_api_key_graceful():
    """settings.OPENAI_API_KEY=None: no OpenAI calls; AI fields get default_value fallback."""
    from app.services.ai_autofill import _autofill
    ...
    with patch("app.services.ai_autofill.get_session_factory") as mock_sf, \
         patch("app.services.ai_autofill.settings") as mock_settings, \
         patch("app.services.ai_autofill.get_object_storage") as mock_storage, \
         patch("app.services.ai_autofill.AsyncOpenAI") as mock_openai_cls, \
         patch("app.services.ai_autofill._write_values", new_callable=AsyncMock) as mock_write, \
         patch("app.services.ai_autofill._set_status", new_callable=AsyncMock):
...
async def test_exception_sets_failed():
    ...
    with patch("app.services.ai_autofill._autofill", new_callable=AsyncMock) as mock_autofill, \
        ...
        mock_autofill.side_effect = RuntimeError("OpenAI connection error")
...
async def test_failed_resets_to_pending():
    ...
    with patch("app.services.ai_autofill.get_session_factory") as mock_sf, \
        ...
        mock_settings.OPENAI_API_KEY = None  # No AI; campaign_name is deterministic
        mock_storage.return_value.download_blob.return_value = (_make_tiny_jpeg(), "image/jpeg")
